...
const EditScheduleModal = ({ isOpen, schedule, onClose, onSuccess }) => {
    ...
    const statuses = [
        { value: "active", label: "Faol", description: "Jadval faol holatda" },
    ...
    const handleSubmit = async (e) => {
        ...
        setLoading(true)
        try {
            const response = await scheduleService.updateSchedule(schedule._id, formData)
            if (response.success) {
            ...
            const message = error.response?.data?.message || "Jadvalni yangilashda xato"
                ...
                <div className="inline-block align-bottom bg-white rounded-xl text-left overflow-hidden shadow-2xl transform transition-all sm:my-8 sm:align-middle sm:max-w-5xl sm:w-full max-h-[90vh] overflow-y-auto">
                    ...
                    <div className="bg-white px-6 pt-6 pb-4 border-b border-gray-200">
                        <div className="flex items-center justify-between">
                            <div>
                                <h3 className="text-xl font-semibold text-gray-900">Jadvalni Tahrirlash</h3>
                                <p className="text-sm text-gray-600 mt-1">{schedule?.name} ma'lumotlarini yangilash</p>
                            </div>
                            <button
                                onClick={onClose}
